...
class QuizElevenController extends Controller
{

    public function onceava()
    {
        $parte_once = file_get_contents('json/parte11.json');
        $preguntas_once = json_decode($parte_once);
        ...
        if ($prueba) {
            ...
            return view('quiz.parte11', compact('preguntas_once', 'arreglo'));
        } else {
            return view('quiz.parte11', compact('preguntas_once'));
        }
    }
    ...
    { 
        ...
        if($request->btnAnterior){ 
            return redirect('/parte10');
        }
        if($request->btnSiguiente){
            return redirect('/parte12');
        }
    }
}
